...
class TextSplitter(BaseChunker, ABC):
    ...
    def from_custom_embedder(
            ...
            chunk_size: int = 6000,  # Conservative limit below 8192
            chunk_overlap: int = 200,
            **kwargs: Any,
    ) -> TS:
        """Text splitter that uses your custom embedder to count tokens."""

        def _embedder_token_length(text: str) -> int:
        ...
        # If this is a CustomEmbedderChunker, pass the embedder instance
        if issubclass(cls, CustomEmbedderChunker):
            kwargs['embedder'] = embedder

        return cls(
            chunk_size=chunk_size,
            chunk_overlap=chunk_overlap,
            length_function=_embedder_token_length,
            **kwargs
        )


class CustomEmbedderChunker(TextSplitter):
    """Text chunker that uses your custom embedder for tokenization."""

    def __init__(
            self,
            embedder,  # Your Embedder instance
            chunk_size: int = 6000,  # Conservative limit below your 8192 max
            chunk_overlap: int = 200,
            **kwargs: Any,
    ) -> None:
        """Create a new CustomEmbedderChunker."""

        # Remove length_function from kwargs if it exists to avoid conflict
        kwargs.pop('length_function', None)

        # Use the embedder's token counting function
        super().__init__(
            chunk_size=chunk_size,
            chunk_overlap=chunk_overlap,
    ...
    def split_text(self, text: str) -> List[str]:
        """Split text using the custom embedder's tokenization."""

        # Create a tokenizer-like object for your embedder
        ...
        return split_text_on_embedder_tokens(text=text, tokenizer=tokenizer)


@dataclass(frozen=True)
class EmbedderTokenizer:
    """Tokenizer wrapper for your custom embedder."""
    ...
    embedder: Any
    """Your custom embedder instance"""

    def encode(self, text: str) -> List[int]:
        """Encode text to token IDs using your embedder."""
        return self.embedder.tokenize_to_ids(text)

    def decode(self, token_ids: List[int]) -> str:
        """Decode token IDs back to text using your embedder."""
        return self.embedder.decode_tokens(token_ids)


def split_text_on_embedder_tokens(*, text: str, tokenizer: EmbedderTokenizer) -> List[str]:
    """Split incoming text and return chunks using your custom embedder tokenizer."""
    splits: List[str] = []
...
if __name__ == "__main__":
    ...
    # Create the chunker with safe token limits
    chunker = CustomEmbedderChunker.from_custom_embedder(
        embedder=embedder,
